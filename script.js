...
const weatherSumaryImg = document.querySelector('.weather-sumary-img')
...
async function updateWeatherInfor(city){
    ...
    await updateForestcastsInfo(city)

    showDisplaySection(weatherInfoSection)
}

async function updateForestcastsInfo(city){
    ...
    forecastData.list.forEach(forecastWeather => {
        if (
            ...
            const temp = Math.round(forecastWeather.main.temp)
            const weatherId = forecastWeather.weather[0].id
            const icon = getWeatherIcon(weatherId)

            const forecastItemHTML = `
                <div class="forecast-item">
                    <h5 class="forecast-item-date regular-txt">${displayDate}</h5>
